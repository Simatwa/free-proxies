...
test_proxy_url = "https://raw.githubusercontent.com/Simatwa/free-proxies/master/files/test.md"
...
def fetch(*args, **kwargs) -> requests.Response:
    return session.get(*args, **kwargs, timeout=request_timeout,
    )

def get_proxies() -> dict[str, list[str]]:
    http_proxies = fetch("https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/http.txt").text.split('\n')
    logging.info(f"Total http proxies {len(http_proxies)}")
    socks4_proxies = fetch("https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/socks5.txt").text.split('\n')
    logging.info(f"Total socks4 proxies {len(socks4_proxies)}")
    socks5_proxies = fetch("https://raw.githubusercontent.com/TheSpeedX/PROXY-List/master/http.txt").text.split("\n")
...
def test_proxy_and_save(proxy_type:str, proxy) -> typing.NoReturn:
    try:
        ...
        if resp.ok:
            logging.info(
                f'Working proxy ({proxy_type}, {proxy})'
                )
